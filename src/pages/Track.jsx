...
export default function Track() {
  ...
  const lookup = async (lookupCode) => {
    if (!lookupCode.trim()) { setError('Please enter an order code.'); return }
    setLoading(true); setError(''); setOrder(null)

    const { data: orderData } = await supabase
      .from('orders').select('*')
      .eq('order_code', lookupCode.trim().toUpperCase()).maybeSingle()
    ...
    setOrder(orderData); setItems(itemsData || []); setLoading(false)
  }
      ...
      <div className="container" style={{ padding: '2.5rem 1.5rem 4rem', maxWidth: 800 }}>
        <form onSubmit={handleSubmit} className="track-form">
          <input
            ...
            placeholder="Enter your order code (e.g. KB-A1B2C3D4)"
          ...
          <div className="track-result">
            <div className="track-header">
              <div>
                ...
                <h2 className="serif">{order.order_code}</h2>
              </div>
              ...
              <div className="track-box">
                ...
                {items.map(it => (
                  <div key={it.id} className="track-item">
                    ...
                    <span className="price serif">{peso(it.subtotal)}</span>
                  </div>
                ))}
...
.step.current .step-dot {
  box-shadow: 0 0 0 4px rgba(200,156,76,.2);
}
.step-label {
...
.track-item .qty { color: var(--gold); font-weight: 500; font-size: .85rem; }
.track-item .name { color: var(--text); }
.track-item .price { color: var(--gold); }
`
