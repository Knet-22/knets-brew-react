...
export function AuthProvider({ children }) {
  ...
  const loadStaff = async (sess) => {
    if (!sess?.user?.email) { setStaff(null); return }
    const { data } = await supabase.from('staff').select('*').eq('email', sess.user.email).maybeSingle()
    setStaff(data)
  }
  ...
  const signIn = async (email, password) => {
    ...
    const { data: staffRow } = await supabase.from('staff').select('*').eq('email', email).maybeSingle()
    if (!staffRow) {
